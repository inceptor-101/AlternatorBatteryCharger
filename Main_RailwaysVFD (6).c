...
int lenNumPoints = 0;
Uint16 i = 0;
Uint16 voltage_values[10];
Uint16 frequency_values[10];
// Pin Out
...
//       EEPROM_WriteByte(enterFreqVoltageLocation, 0xFF);
//       EEPROM_WriteByte(enterFreqVoltageLocation+1, 0xFF);
       ...
       if(EEPROM_ReadByte(ratedVoltageLocation) == 0xFF && EEPROM_ReadByte(ratedVoltageLocation+1) == 0xFF)
        ...
        {
            if(EEPROM_ReadByte(ratedVoltageLocation+26+((caliState-151)*2)) == 0xFF && EEPROM_ReadByte(ratedVoltageLocation+27+((caliState-151)*2)) == 0xFF && EEPROM_ReadByte(ratedVoltageLocation+28+((caliState-151)*2)) == 0xFF)
            {
           ...
           else
           {
              ...
              eepromcalibrationFactor[caliState-151] = N2 + (N1/100.0);
           }
        }
        ...
        for(numPoints =0; numPoints < totalPoints; numPoints++)
        {
           ...
           {
               O2 = EEPROM_ReadByte(enterFreqVoltageLocation + (numPoints*4));             //reads voltage points
               O1 = EEPROM_ReadByte(enterFreqVoltageLocation +1 + (numPoints*4));
           ...
           {
               P2 = EEPROM_ReadByte(enterFreqVoltageLocation +2 + (numPoints*4));             //reads frequency points
               P1 = EEPROM_ReadByte(enterFreqVoltageLocation +3 + (numPoints*4));
                            ...
                            if(enter==0)
                            {
                                ...
                                if(enterCounter == 500)
                                {
                                    ...
                                    if(calibrationFactor[caliState-151]>=0.9 && calibrationFactor[caliState-151]<=1.1)
                                    {
                                        ...
                                        vryCalFactor = calibrationFactor[0];
                                        vrbCalFactor = calibrationFactor[1];
                                        VdcCalFactor = calibrationFactor[2];
                                        irCalFactor = calibrationFactor[3];
                                        iyCalFactor = calibrationFactor[4];
                                        ibCalFactor = calibrationFactor[5];
                                 ...
                                 if(enter==0)
                                 {
                                     ...
                                     if(enterCounter == 500)
                                     {
                                         ...
                                         if(voltageInt > 0 && voltageInt < (int)ratedMotorVoltage && numPoints == 0)
                                         {
                                             ...
                                             EEPROM_WriteByte(enterFreqVoltageLocation+(numPoints*4), O2);
                                             EEPROM_WriteByte(enterFreqVoltageLocation+1+(numPoints*4), O1);
                          ...
                          case points_1_freq:
                            {
                                if(EEPROM_ReadByte(enterFreqVoltageLocation) == 0xFF && EEPROM_ReadByte(enterFreqVoltageLocation+1) == 0xFF)
                                  ...
                                  else
                                  {
                                      P2 = EEPROM_ReadByte(enterFreqVoltageLocation +2+ (numPoints*4));             //reads frequency points
                                      P1 = EEPROM_ReadByte(enterFreqVoltageLocation +3 + (numPoints*4));
                                      eepromVoltagePoints[numPoints] = ((Uint16) P1) | ((Uint16) (P2 << 8));
                                   //   voltage_values[numPoints] = eepromVoltagePoints[numPoints];
                                  }
                                while(1)
                                {
                                  ...
                                  if(up==0)
                                   ...
                                   if(enterCounter == 500)
                                   {
                                       ...
                                       else if(freqInt<(int)ratedMotorFrequency && freqInt > frequency_values[numPoints-1] && totalPoints>1)
                                       {
                                           ...
                                           P2 = (frequency_values[numPoints]>>8);
                                           P1 = (frequency_values[numPoints] & 0x00FF);
                                           EEPROM_WriteByte(enterFreqVoltageLocation+2+(numPoints*4), P2);
                                           EEPROM_WriteByte(enterFreqVoltageLocation+3+(numPoints*4), P1);
